...
export async function analyzeEmailAsync(
  ...
  // ── 4. Live IP geo/ASN lookup ──────────────────────────────────────────
  if (liveGeoEnabled && base.origin.ip && base.origin.ipClass === "public") {
    try {
      ...
      if (geoRes.ok) {
        ...
        if (!geo.error && geo.country_name) {
          ...
          // Mutate origin in the base analysis with live data (higher confidence)
      ...
      // ipapi.co unavailable — NET_MAP/HOST_HINTS static data already applied
    }
  }
