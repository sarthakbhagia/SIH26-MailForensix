...
export async function analyzeEmailAsync(
  ...
  // ── 4. Live ML Engine & IP geo/ASN lookup — adapted to backend ingestion/analysis
  try {
    const formData = new FormData();
    const blob = new Blob([raw], { type: "message/rfc822" });
    formData.append("file", blob, "email.eml");

    const backend = (typeof window !== 'undefined' && import.meta.env?.VITE_API_BASE) ? import.meta.env.VITE_API_BASE : 'http://localhost:8000';

    // Upload email to backend ingestion endpoint
    const uploadRes = await fetch(`${backend}/api/emails/upload`, {
      method: 'POST',
      body: formData,
    });
    ...
    if (uploadRes.ok) {
      ...
      const emailId = uploadData?.email_id || uploadData?.emailId || uploadData?.email_id;
      if (emailId) {
        // Poll for analysis result (simple backoff)
        for (let attempt = 0; attempt < 10; attempt++) {
          await new Promise((r) => setTimeout(r, 1000 * (attempt + 1)));
          try {
            const analysisRes = await fetch(`${backend}/api/analysis/${emailId}`);
            if (!analysisRes.ok) continue;
            const analysis = await analysisRes.json();
            if (!analysis) continue;

            // Map geo_data
            if (analysis.geo_data && analysis.geo_data.length > 0) {
              const geo = analysis.geo_data[0];
              if (geo.country) {
                base.origin.country = geo.country;
                base.origin.region = geo.region || base.origin.region;
                base.origin.provider = geo.org || base.origin.provider || geo.isp;
                if (geo.latitude) base.origin.latitude = geo.latitude;
                if (geo.longitude) base.origin.longitude = geo.longitude;
                base.origin.confidence = geo.confidence || 85;
                base.origin.basis.push(`Backend analysis: ${geo.org || geo.isp || 'ISP'} · ${geo.country}`);
              }
            }

            // Map relay_path originating ip
            if (analysis.relay_path && analysis.relay_path.length > 0) {
              const firstHop = analysis.relay_path[0];
              if (firstHop?.ip) base.origin.ip = firstHop.ip;
            }

            // Map IOC items into findings
            if (analysis.iocs && Array.isArray(analysis.iocs)) {
              for (const item of analysis.iocs) {
                extraFindings.push({
                  id: `backend-ioc-${Math.random().toString(36).slice(2, 7)}`,
                  title: item.type ? `IOC: ${item.type}` : 'IOC',
                  detail: item.reason || item.value || JSON.stringify(item),
                  severity: item.risk_score && item.risk_score > 50 ? 'high' : 'info',
                  category: 'infrastructure',
                  weight: item.risk_score ? Math.min(20, Math.round(item.risk_score)) : 2,
                });
              }
            }

            // Map NLP result into findings
            if (analysis.nlp_result && analysis.nlp_result.label) {
              extraFindings.push({
                id: `backend-nlp-${Math.random().toString(36).slice(2,7)}`,
                title: `NLP: ${analysis.nlp_result.label}`,
                detail: `Confidence ${analysis.nlp_result.confidence}`,
                severity: analysis.nlp_result.confidence > 0.7 ? 'high' : 'info',
                category: 'content',
                weight: analysis.nlp_result.confidence ? Math.round(analysis.nlp_result.confidence * 10) : 2,
              });
            }

            // Found usable analysis — stop polling
            break;
          } catch (e) {
